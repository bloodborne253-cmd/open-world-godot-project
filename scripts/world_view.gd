...
func _get_chest_sheet_texture(tile_id: int) -> Texture2D:
	...
	atlas.atlas = sheet
	# Safe static icon from the sheet: closed brown chest (top-left cell).
	# For now both closed/open tile IDs use the same source art so we avoid bad crops.
		...
		for y in range(grid_h):
			...
			for x in range(grid_w):
				...
				if obj != cm.OBJECT_NONE:
					if bool(main_ref.has_method("use_scene_chests") and main_ref.use_scene_chests()) and obj in [cm.TILE_CHEST_CLOSED, cm.TILE_CHEST_OPEN]:
						pass
					elif not _draw_tile_visual(px, py, ts, obj):
