...
@onready var chest_layer: Node2D = $ChestInstances
@onready var pickup_layer: Node2D = $PickupInstances
@onready var player: Player = $Player
@onready var camera: Camera2D = $Camera2D
@onready var hud_panel: Panel = $CanvasLayer/HUDPanel
@onready var hearts_label: Label = $CanvasLayer/HUDPanel/HeartsLabel
@onready var coins_label: Label = $CanvasLayer/HUDPanel/CoinsLabel
@onready var area_label: Label = $CanvasLayer/HUDPanel/AreaLabel
@onready var status_label: Label = $CanvasLayer/HUDPanel/StatusLabel
...
var _skip_build_prev_shortcut_once: bool = false
var _skip_build_next_shortcut_once: bool = false
...
func _input(event: InputEvent) -> void:
	...
	if _handle_build_mode_controller_overrides(event):
		get_viewport().set_input_as_handled()
		return
...
func _handle_build_mode_controller_overrides(event: InputEvent) -> bool:
	if not build_mode:
		return false
	...
	if not button_event.pressed:
		return false
	match int(button_event.button_index):
		2:
			_toggle_build_palette()
			return true
		9:
			if palette_open:
				...
				_skip_build_prev_shortcut_once = true
		...
		10:
			if palette_open:
				...
				_skip_build_next_shortcut_once = true
				_change_build_item(1)
			return true
	return false
...
func _handle_build_palette_shortcuts() -> void:
	if _skip_build_prev_shortcut_once:
		_skip_build_prev_shortcut_once = false
	elif Input.is_action_just_pressed("build_prev"):
		_change_build_item(-1)

	if _skip_build_next_shortcut_once:
		_skip_build_next_shortcut_once = false
...
func use_scene_chests() -> bool:
	return true

func try_open_chest_at_world(world_pos: Vector2) -> bool:
	if chunk_manager.try_open_chest_at_world(world_pos):
		_sync_chest_instances()
		_spawn_chest_reward(world_pos)
		world.queue_redraw()
		return true
	return false

func _spawn_chest_reward(world_pos: Vector2) -> void:
	if not player.is_full_health() and randf() < 0.45:
		_spawn_pickup("heart", world_pos + Vector2(0, -10), 1, Vector2(0, -40))
		show_status("Chest opened: heart")
		return
	var coin_count: int = 5
	for i in range(coin_count):
		var angle: float = (-0.8 + i * 0.4)
		var drift := Vector2(cos(angle), sin(angle) - 0.9) * 90.0
		_spawn_pickup("coin", world_pos + Vector2(0, -6), 1, drift)
	show_status("Chest opened: +5 coins")

func _spawn_random_world_drop(world_pos: Vector2, source_tile_id: int = -1) -> void:
	var heart_chance: float = 0.18 if source_tile_id == chunk_manager.TILE_POT else 0.08
	var coin_chance: float = 0.55 if source_tile_id == chunk_manager.TILE_POT else 0.32
	var roll: float = randf()
	if not player.is_full_health() and roll < heart_chance:
		_spawn_pickup("heart", world_pos, 1, Vector2(randf_range(-40.0, 40.0), randf_range(-60.0, -20.0)))
	elif roll < heart_chance + coin_chance:
		_spawn_pickup("coin", world_pos, 1, Vector2(randf_range(-50.0, 50.0), randf_range(-55.0, -15.0)))

func _spawn_pickup(kind: String, world_pos: Vector2, amount: int = 1, drift: Vector2 = Vector2.ZERO) -> void:
...
func _sync_chest_instances() -> void:
	...
	for coord in visible_chunk_coords_cache:
		var chunk: Dictionary = chunk_manager.get_chunk(coord)
		var origin: Vector2 = chunk_manager.chunk_origin(coord)
		...
		for y in range(grid_h):
			var row: Array = objects[y]
			for x in range(grid_w):
				var tile_id: int = int(row[x])
				if tile_id != chunk_manager.TILE_CHEST_CLOSED and tile_id != chunk_manager.TILE_CHEST_OPEN:
					continue
				var world_pos: Vector2 = origin + Vector2((x + 0.5) * TILE_SIZE, (y + 0.5) * TILE_SIZE)
				var key: String = "%d,%d" % [int(world_pos.x), int(world_pos.y)]
				desired[key] = true
				var chest: Node = chest_instances.get(key, null)
				if chest == null:
					chest = CHEST_SCENE.instantiate()
					chest_layer.add_child(chest)
					chest_instances[key] = chest
				if chest.has_method("setup_for_world"):
					chest.setup_for_world(self, world_pos, tile_id == chunk_manager.TILE_CHEST_OPEN)
	for key in chest_instances.keys():
		if desired.has(key):
			continue
		var chest: Node = chest_instances[key]
		if is_instance_valid(chest):
			chest.queue_free()
		chest_instances.erase(key)
...
func _layout_hud() -> void:
	var view_size: Vector2 = get_viewport_rect().size
	var panel_w: float = minf(760.0, view_size.x - 24.0)
	hud_panel.position = Vector2(12.0, 10.0)
	hud_panel.size = Vector2(panel_w, 54.0)
	hearts_label.position = Vector2(14.0, 8.0)
	hearts_label.size = Vector2(210.0, 38.0)
	coins_label.position = Vector2(228.0, 8.0)
	coins_label.size = Vector2(130.0, 38.0)
	area_label.position = Vector2(362.0, 8.0)
	area_label.size = Vector2(180.0, 38.0)
	status_label.position = Vector2(548.0, 8.0)
	status_label.size = Vector2(maxf(120.0, panel_w - 562.0), 38.0)
...
func _update_hud() -> void:
	...
	var theme: String = str(chunk.get("theme", "field")).capitalize()
	var hearts: String = ""
	for i in range(player.max_health):
		hearts += "♥" if i < player.health else "♡"
	hearts_label.text = hearts
	coins_label.text = "◉ %d" % player.coins
	area_label.text = "%s  %s" % [theme, str(current_chunk)]
	...
	if build_mode:
		...
		area_label.text = "Build %s (%d,%d)" % [str(cchunk), clocal.x, clocal.y]
		status_line = "%s  %dx%d" % [get_current_build_tile_name(), build_brush_size, build_brush_size] + (("  " + status_text) if status_text != "" else "")
	status_label.text = status_line
	debug_label.text = "Loaded: %d   Visible: %d\nPos: (%.0f, %.0f)   Tile under: %d   Chunk %s\nTheme: %s   Local tile: (%d,%d)   Build page: %s   Palette: %s" % [chunk_manager.get_loaded_chunks().size(), visible_chunk_coords_cache.size(), player_world_pos.x, player_world_pos.y, chunk_manager.get_tile_at_world(player_world_pos), str(current_chunk), theme, local.x, local.y, _get_current_page_name(), "OPEN" if palette_open else "CLOSED"]
...
func _ensure_input_actions() -> void:
	...
	_bind_joy_button("build_palette", 10)
